...
import static cui.shibing.commonrepository.iml.mybatis.MybatisCommonMapper.*;
...
public class MybatisCommonRepositoryImpl<T, ID> implements CommonRepository<T, ID> {
    ...
    public void postInit() {
        ...
        if (!configuration.hasMapper(MybatisCommonMapper.class)) {
            configuration.addMapper(MybatisCommonMapper.class);
        }
    ...
    public List<T> findAll() {
        ...
        try (SqlSession session = sessionFactory.openSession()) {
            ...
            parameterMap.put(PARAM_TABLE, tableName);
            parameterMap.put(PARAM_COLUMNS, EntityInfoHolder.getEntityInfo(domainClass, isMapUnderscoreToCamelCase).getAllColumnNames());
            CommonListResultHandler<T> commonResultHandler = new CommonListResultHandler<>(configuration, domainClass);
            session.select(FIND_ALL, parameterMap, commonResultHandler);
